...
def extract_from_kaggle(**context):
    ...
    source_file = os.path.join(path, "Airbnb_Open_Data.csv")
    dest_file = os.path.join(DATA_DIR, "Airbnb_Open_Data.csv")
...
def load_raw_to_postgres(**context):
    ...
    CREATE TABLE raw_airbnb (
        id SERIAL PRIMARY KEY, neighbourhood_group TEXT, room_type TEXT, price TEXT,
        number_of_reviews DECIMAL(10,2), availability_365 DECIMAL(10,2),
...
def transform_and_clean_data(**context):
    ...
    df['price'] = df['price_clean'].astype('float64')
